...
function agregarAlEquipo(nombre) {
	...
	const pokemonValido = pokedex.find(
		(p) => normalizarNombre(p) === entradaNormalizada
	);
...
function simuladorInteractivo() {
	alert(
		"¡Bienvenido! Esta Pokédex contiene los 151 Pokémon de la primera generación."
	);

	while (equipoPokemon.length < 6) {
		const nombre = prompt(
			"Ingresa el nombre de un Pokémon para agregar (o escribe 'salir'):"
		);
...
alert(
	"Simulador de Equipo Pokémon. Puedes ver el resultado en la consola."
);
